...
def Message processData(Message message) {
    Logger log = LoggerFactory.getLogger(this.getClass());
     ...
     try {
        processData("oauth_proxy_pe_01", message);
    } catch (Exception ex) {
...
def void processHeadersAndProperties(String prefix, Message message) {
    ...
    try {
        ...
        sb_html.append("<h1>Body</h1><br>\n");
        sb_html.append("<table>\n");
        sb_html.append("<tr><td>Body</td><td>"+body_test+"</td></tr>\n");
        if (body_test!=null) {
          sb_html.append("<tr><td>CanonicalClassName</td><td>"+body_test.getClass().getCanonicalName()+"</td></tr>\n");
        }
        sb_html.append("</table>\n");     
